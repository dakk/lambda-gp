...
let rand_var2 v =  String.get var_alphabet (Random.int v) |> String.make 1;;
...
let rec rand_term d v = match (d, Random.int 3) with
| (0, _) -> Var (rand_var v);
| (_, 0) -> Abs (rand_var v, rand_term (d-1) v)
| (_, 1) -> App (rand_term (d-1) v, rand_term (d-1) v)
| (_, 2) -> Var (rand_var v);
;;

(** Generate random lambda term with a fixed length of l terms using v free variables *)
let rand_term_l l v = 
  let rec rin l' = match (l - l', Random.int 3) with 
